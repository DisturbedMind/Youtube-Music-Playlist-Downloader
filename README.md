...
Cleanup is limited to the app-created `_staging` folder. The downloader purges `_staging` after every playlist attempt so the temporary staging directory does not remain visible. It must not scan or delete existing artist, album, or playlist folders in your output library.
